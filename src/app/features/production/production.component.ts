...
@Component({
  ...
  imports: [
    ...
    PageHeaderComponent,
    MatCardModule, MatButtonModule, MatInputModule, MatSelectModule, MatIconModule
  ],
  template: `
      ...
      <div class="panel mb-6" style="max-width:480px;">
        ...
        <mat-form-field class="w-full" appearance="outline">
          ...
          <mat-select [value]="selectedBandId()" (selectionChange)="onBandSelect($event.value)">
            <mat-option *ngFor="let band of activeBands()" [value]="band.id">
              {{ band.name }} — {{ band.status }}
            </mat-option>
          </mat-select>
        </mat-form-field>
      </div>

      <div *ngIf="selectedBand() as band" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <!-- Form -->
        <div>
          <!-- Saillie -->
          <div *ngIf="band.status === 'reproduction'" class="panel">
            <p class="panel__title"><mat-icon>favorite_border</mat-icon> Enregistrer la saillie</p>
            <form [formGroup]="saillieForm" (ngSubmit)="submitSaillie(band.id)" class="flex flex-col gap-4">
              <mat-form-field class="w-full" appearance="outline">
                <mat-label>Date de la saillie</mat-label>
                <input matInput type="date" formControlName="dateBreeding">
              </mat-form-field>
              <button mat-flat-button color="primary" type="submit" [disabled]="saillieForm.invalid"
                style="width:100%; border-radius:8px; height:42px;">Valider</button>
            </form>
          </div>

          <!-- Mise bas -->
          <div *ngIf="band.status === 'gestation'" class="panel">
            <p class="panel__title"><mat-icon>child_friendly</mat-icon> Enregistrer la mise bas</p>
            <form [formGroup]="miseBasForm" (ngSubmit)="submitMiseBas(band.id)" class="flex flex-col gap-4">
              <mat-form-field class="w-full" appearance="outline">
              ...
              <button mat-flat-button color="primary" type="submit" [disabled]="miseBasForm.invalid"
                style="width:100%; border-radius:8px; height:42px;">Valider</button>
            </form>
          </div>

          <!-- Sevrage -->
          <div *ngIf="band.status === 'sevrage'" class="panel">
            <p class="panel__title"><mat-icon>content_cut</mat-icon> Enregistrer le sevrage</p>
            <form [formGroup]="sevrageForm" (ngSubmit)="submitSevrage(band.id)" class="flex flex-col gap-4">
              <mat-form-field class="w-full" appearance="outline">
              ...
              <mat-form-field class="w-full" appearance="outline">
                ...
                <input matInput type="number" step="0.01" formControlName="avgWeightAtWeaning">
              </mat-form-field>
              <button mat-flat-button color="primary" type="submit" [disabled]="sevrageForm.invalid"
                style="width:100%; border-radius:8px; height:42px;">Valider</button>
            </form>
          </div>

          <!-- Engraissement -->
          <div *ngIf="band.status === 'engraissement'" class="panel">
            <p class="panel__title"><mat-icon>storefront</mat-icon> Finaliser l'engraissement</p>
            <form [formGroup]="engraissementForm" (ngSubmit)="submitEngraissement(band.id)" class="flex flex-col gap-4">
              <mat-form-field class="w-full" appearance="outline">
              ...
              <button mat-flat-button color="primary" type="submit" [disabled]="engraissementForm.invalid"
                style="width:100%; border-radius:8px; height:42px;">Terminer et archiver</button>
            </form>
          </div>
        </div>
            ...
            <table class="data-table">
              <tbody>
                ...
                <tr>
                  ...
                  <td>{{ currentWeaning()?.avgWeightAtWeaning ? currentWeaning()?.avgWeightAtWeaning + ' kg' : '—' }}</td>
                </tr>
                <tr>
...
export class ProductionComponent {
  ...
  selectedBand = computed(() => this.farmService.bands().find(b => b.id === this.selectedBandId()));
  currentReproduction = computed(() => this.farmService.reproductions().find(r => r.bandId === this.selectedBandId()));
  ...
  submitSaillie(bandId: string): void {
    if (this.saillieForm.valid) {
      this.farmService.addReproduction({ bandId, dateBreeding: this.saillieForm.value.dateBreeding });
  ...
  submitEngraissement(bandId: string): void {
    if (this.engraissementForm.valid) {
      ...
      if (fattening) {
        ...
        const revenue = Number(this.engraissementForm.value.nbSold) * Number(this.engraissementForm.value.avgWeight) * 2800;
        this.farmService.addSale({
          fatteningId: fattening.id, dateOrder: new Date().toISOString().split('T')[0],
          customer: 'Centragel', nbRequested: Number(this.engraissementForm.value.nbSold),
          nbDelivered: Number(this.engraissementForm.value.nbSold), pricePerKg: 2800, totalAmount: revenue
        });
