    ...
    <div class="page-container">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div>
          <div class="panel">
            ...
            <form [formGroup]="bandForm" (ngSubmit)="onSubmit()" class="flex flex-col gap-4">
              ...
              <button mat-flat-button color="primary" type="submit" [disabled]="bandForm.invalid"
        ...
        <div class="lg:col-span-2">
          ...
          <app-empty-state *ngIf="bands().length === 0" icon="layers" message="Aucune bande creee. Ajoutez-en une pour commencer le suivi."></app-empty-state>

          <div class="flex flex-col gap-4">
            <div *ngFor="let band of bands()" class="panel">
              <!-- Band Header -->
              <div class="flex justify-between items-start mb-4">
                <div>
                  <h3 style="font-size:15px; font-weight:650; color:#1e293b; margin:0 0 2px;">{{ band.name }}</h3>
                  <p style="font-size:12px; color:#94a3b8; margin:0;">Creee le {{ band.dateCreated }} — {{ band.nbFemales }} femelles</p>
                </div>
                <div class="flex items-center gap-2">
                  <span class="badge" [ngClass]="{
                    'badge--neutral': band.status === 'archived',
                    'badge--info': band.status === 'reproduction' || band.status === 'gestation',
                    'badge--warning': band.status === 'sevrage',
                    'badge--success': band.status === 'engraissement'
                  }">{{ band.status }}</span>
                  <button mat-icon-button (click)="deleteBand(band.id)" style="width:32px;height:32px;">
                    <mat-icon style="font-size:18px;width:18px;height:18px;color:#94a3b8;">delete_outline</mat-icon>
                  </button>
                </div>
              </div>
              ...
              <div style="padding:8px 0;">
                <div class="flex items-center justify-between relative">
                  <div style="position:absolute;left:16px;right:16px;top:16px;height:2px;background:#e2e8f0;z-index:0;"></div>
                  ...
                  <div class="timeline-step" *ngFor="let step of timelineSteps">
                    <div class="timeline-dot"
                      ...
                      [class.timeline-dot--pending]="!isStepDone(band, step.key) && !isStepActive(band, step.key)">
                      <mat-icon style="font-size:14px;width:14px;height:14px;">{{ step.icon }}</mat-icon>
                    </div>
              ...
              <!-- Notes -->
              <p *ngIf="band.notes" style="font-size:12px; color:#64748b; margin:12px 0 0; padding:8px 12px; background:#f8fafc; border-radius:6px; border-left:3px solid #e2e8f0; font-style:italic;">
                {{ band.notes }}
              </p>
            </div>
          </div>
...
export class BandesComponent {
  ...
  isStepDone(band: Band, step: string): boolean {
    ...
    const stepMap: Record<string, number> = {
      saillie: 0, gestation: 1, mise_bas: 2, sevrage: 3, engraissement: 3, vente: 4
    };
  ...
  isStepActive(band: Band, step: string): boolean {
    const map: Record<string, string> = {
      saillie: 'reproduction', gestation: 'gestation', mise_bas: 'sevrage',
      sevrage: 'engraissement', engraissement: 'engraissement', vente: 'archived'
    };
